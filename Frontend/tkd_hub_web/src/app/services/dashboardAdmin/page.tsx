...
const DashboardAdminPage = () => {
  ...
  const [widgets, setWidgets] = useState<string[]>(["ActiveMembers", "Revenue"]);
  const [response, setResponse] = useState<{ data: Record<string, unknown> } | null>(null);
    ...
    <div className="max-w-xl mx-auto mt-10 p-6 bg-white rounded shadow">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <input
          ...
          value={widgets.join(",")}
          onChange={e => setWidgets(e.target.value.split(",").map(w => w.trim()))}
        />
        <button
          type="submit"
