...
export default function DojaangAdmin() {
  ...
  useEffect(() => {
    const fetchList = async () => {
      ...
      setError(null)
      try {
        const res = await fetch(`${baseUrl}/Dojaang`, { headers: { Authorization: token ? `Bearer ${token}` : '' } })
  ...
  const handleDelete = async (id: number) => {
    ...
    setDeleteId(id)
    try {
      const res = await fetch(`${baseUrl}/Dojaang/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } })
  ...
  {error && <Alert severity="error">{error}</Alert>}
          ...
          {dojaangs.map(d => (
            <tr key={d.id} className="border-t">
              ...
              <td className="px-4 py-2">
                <button title="Details" className="mr-2 p-1" onClick={() => setEditId(d.id)}>
                  <DescriptionIcon />
                </button>
                {Array.isArray(role) && role.includes('Admin') && (
                  <button title="Delete" className="text-red-600 p-1" onClick={() => handleDelete(d.id)}>
                    <DeleteIcon />
                  </button>
                )}
              </td>
      ...
      {editId && <EditDojaang dojaangId={editId} onClose={() => setEditId(null)} />}
    </div>
  )
}
