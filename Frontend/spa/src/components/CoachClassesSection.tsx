...
export const CoachClassesSection: React.FC = () => {
  ...
  const handleAttendanceSubmit = async (studentId: number, present: boolean) => {
    if (!selectedClass) return;
    
    const success = await markAttendance(selectedClass.id, studentId, present, attendanceNotes);
    if (success) {
  ...
  const getUpcomingClasses = () => {
    ...
    return managedClasses.filter(cls => {
          ...
          <Card>
            <CardContent sx={{ textAlign: 'center' }}>
              ...
              <Typography variant="h4" color="success.main">
                {managedClasses.reduce((total, cls) => total + (cls.enrolledStudents?.length || 0), 0)}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Total Students
              </Typography>
            ...
            <Grid container spacing={2}>
              {upcomingClasses.map((cls) => (
                <Grid item xs={12} md={4} key={cls.id}>
                  <Card variant="outlined" sx={{ cursor: 'pointer' }} onClick={() => navigate(`/classes/${cls.id}`)}>
