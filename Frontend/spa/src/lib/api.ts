  ...
  try {
    ...
    try {
      ...
      if (text && text.trim().startsWith('<')) {
        throw new ApiError(`Expected JSON but received HTML response (status ${res.status}). Response starts with: ${text.trim().slice(0,100)}`, res.status, text)
      }
      ...
      throw new ApiError(`Expected JSON but response body could not be read (status ${res.status})`, res.status)
    }
  }
}
