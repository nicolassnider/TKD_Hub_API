...
export default function LoginForm() {
  ...
  const roleArray = role ? (Array.isArray(role) ? role : [role]) : ['Student']
  setRole(roleArray)
  ...
  {error && <Alert severity="error" className="mb-2">{error}</Alert>}
      <div className="mb-3">
        ...
        <input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)} className="mt-1 block w-full border rounded p-2" placeholder="you@example.com" required />
      </div>
      <div className="mb-3">
        <label htmlFor="password" className="block text-sm font-medium text-gray-700">Password</label>
        <input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} className="mt-1 block w-full border rounded p-2" placeholder="Your password" required />
      </div>
