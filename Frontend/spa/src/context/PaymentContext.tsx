...
import { CreatePreferenceDTO, CreatePreferenceResponse } from '../lib/paymentTypes';
...
export const PaymentProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const apiRequest = useCallback(async <T,>(url: string, opts?: RequestInit): Promise<T> => {
  ...
  const createPreference = useCallback(async (dto: CreatePreferenceDTO): Promise<CreatePreferenceResponse> => {
    ...
    setError(null);
    try {
      const data = await apiRequest<CreatePreferenceResponse>('/MercadoPago/create-preference', {
        method: 'POST',
