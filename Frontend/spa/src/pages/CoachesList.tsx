...
function CoachesTable() {
	const { items, loading, error } = useApiItems('/api/Coaches')
		...
		const { items: rows, loading: tableLoading, error: tableError, reload } = useApiItems('/api/Coaches')
		const cols = [
		...
		return (
			<div>
				...
				<ApiTable rows={rows} columns={cols} onRowClick={(r) => navigate(`/coaches/${r.id}`)} defaultPageSize={5} pageSizeOptions={[5,10,25]} />
			</div>
		)
}
