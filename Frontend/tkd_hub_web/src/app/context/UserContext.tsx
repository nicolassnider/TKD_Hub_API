...
export const UserProvider = ({ children }: { children: ReactNode }) => {
  ...
  // --- GET /api/Users ---
  // Add page and pageSize parameters with defaults
  const fetchUsers = useCallback(
    async (page: number = 1, pageSize: number = 10) => {
      setLoading(true);
      setError(null);
      try {
        // Pass pagination params as query
        ...
        const usersArray = Array.isArray(response?.data?.items)
          ? response.data.items
          : [];
        setUsers(usersArray);
        ...
        // Optionally, you can store totalCount, page, pageSize in state if needed for UI
      } catch (err) {
  ...
  const getUserById = useCallback(
    ...
    [apiRequest]
  ); // Dependency array ensures the function is stable unless apiRequest changes.
  ...
  const updateUser = useCallback(
    async (id: number, user: Partial<User>) => {
      setLoading(true);
      setError(null);
      try {
