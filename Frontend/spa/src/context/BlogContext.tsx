...
export const BlogProvider: React.FC<BlogProviderProps> = ({ children }) => {
  ...
  const fetchPosts = useCallback(async () => {
    try {
      ...
      const errorMessage =
        err instanceof ApiError ? err.message : "Failed to fetch blog posts";
  ...
  const fetchPost = useCallback(
    async (id: number): Promise<BlogPost | null> => {
      try {
        ...
        const errorMessage =
          err instanceof ApiError ? err.message : "Failed to fetch blog post";
        setError(errorMessage);
  ...
  const createPost = useCallback(
    async (data: CreateBlogPostDto): Promise<BlogPost> => {
      try {
        setLoading(true);
        setError(null);
  ...
  const updatePost = useCallback(
    async (id: number, data: UpdateBlogPostDto): Promise<BlogPost> => {
      try {
        setLoading(true);
        setError(null);
  ...
  const getPermissions = useCallback(
    (post?: BlogPost): BlogPermissions => {
      ...
      const canEdit =
        isAdmin() ||
        (hasRole("Coach") &&
          post?.authorId === parseInt(token?.split(".")[1] || "0"));
