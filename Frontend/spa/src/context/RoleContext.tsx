...
export const RoleProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (r) {
      try {
        ...
        else if (typeof parsed === 'string' && parsed.length > 0) setRoleState([parsed])
      } catch {
  ...
  return (
    <RoleContext.Provider value={{ token, role, displayName, avatarUrl, setToken, setRole, getRole, setDisplayName, setAvatarUrl, roleLoading }}>
      {children}
