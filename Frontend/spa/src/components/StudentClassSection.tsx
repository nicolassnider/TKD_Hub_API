...
export const StudentClassSection: React.FC = () => {
  ...
  const formatScheduleDisplay = (schedules: any[]) => {
    ...
    return schedules
      .map(schedule => {
        const day = DAYS_OF_WEEK.find(d => d.value === schedule.day)?.label || 'Unknown';
  ...
  const getNextScheduleTime = () => {
    if (!enrolledClass?.schedules || enrolledClass.schedules.length === 0) return null;
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const schedule = enrolledClass.schedules.find(s => {
        ...
        return scheduleDay === targetDay && (i > 0 || s.startTime > currentTime);
      });
      ...
      if (schedule) {
        ...
        return {
          ...
          day: DAYS_OF_WEEK.find(d => d.value === schedule.day)?.label || 'Unknown'
        };
      }
    }
    ...
    return (
      <Box sx={{ textAlign: 'center', py: 8 }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
          You're not currently enrolled in any training class. Contact your instructor or admin to get enrolled.
        </Typography>
        <Button
          variant="outlined"
  ...
  return (
    <Box sx={{ p: 3 }}>
        ...
        <CardContent>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              ...
              {/* Payment Status */}
              {studentProfile && (
                <Card variant="outlined" sx={{ bgcolor: nextPayment ? 'warning.50' : 'success.50' }}>
                  <CardContent sx={{ py: 2, '&:last-child': { pb: 2 } }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                      <PaymentIcon color={nextPayment ? 'warning' : 'success'} />
                      <Typography variant="subtitle2" color={nextPayment ? 'warning.main' : 'success.main'}>
                        Payment Status
                      </Typography>
                    </Box>
                    <Chip
                      label={studentProfile.paymentStatus}
                      color={getPaymentStatusColor(studentProfile.paymentStatus) as any}
                      size="small"
                    />
                    {nextPayment && (
      ...
      {/* Classmates */}
      {enrolledClass.enrolledStudents && enrolledClass.enrolledStudents.length > 0 && (
        <Card>
          <CardContent>
            ...
            <List>
              {enrolledClass.enrolledStudents.slice(0, 5).map((student, index) => (
                <React.Fragment key={student.id}>
                  ...
                  <ListItem sx={{ px: 0 }}>
                    ...
                    <ListItemText
                      ...
                      secondary={student.dojaangName && `From ${student.dojaangName}`}
                    />
                  </ListItem>
                </React.Fragment>
              ))}
              ...
              {enrolledClass.enrolledStudents.length > 5 && (
                <ListItem sx={{ px: 0, textAlign: 'center' }}>
                  <ListItemText>
                    <Typography variant="body2" color="text.secondary">
                      And {enrolledClass.enrolledStudents.length - 5} more students...
                    </Typography>
                  </ListItemText>
                </ListItem>
              )}
