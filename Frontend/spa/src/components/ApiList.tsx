...
export default function ApiList({ apiPath, idField = 'id', titleField = 'name' }: Props) {
        ...
        <ul className="space-y-2">
          {items.map((it: any) => (
            <li key={it[idField]} className="p-2 border rounded">
              <Link to={`${apiPath.replace('/api/', '/').toLowerCase()}/${it[idField]}`} className="text-blue-600 font-medium">
