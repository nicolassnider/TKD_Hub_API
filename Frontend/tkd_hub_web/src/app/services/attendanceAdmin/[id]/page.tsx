...
export default function AttendanceAdminPage({ params }: Props) {
    ...
    const { getClassById, getStudentAttendanceHistory, loading, error } = useClasses();

    // Use explicit types instead of any
    const [classData, setClassData] = useState<ClassData | null>(null);
    const [attendanceHistory, setAttendanceHistory] = useState<StudentClassAttendance[]>([]);
    ...
    return (
        <main className="p-6">
            <h1 className="text-2xl font-bold mb-4">Attendance Admin - Class ID: {id}</h1>
            <pre className="bg-gray-100 p-4 rounded mb-6">{JSON.stringify(classData, null, 2)}</pre>
            <h2 className="text-xl font-semibold mb-2">Student Attendance History</h2>
            <pre className="bg-gray-50 p-4 rounded">{JSON.stringify(attendanceHistory, null, 2)}</pre>
        </main>
    );
}
