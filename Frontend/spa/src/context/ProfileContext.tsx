...
interface ProfileContextType {
  ...
  markAttendance: (classId: number, studentId: number, present: boolean, notes?: string) => Promise<boolean>;
...
export const ProfileProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // State
  const [profile, setProfile] = useState<UserProfile | CoachProfile | StudentProfile | CombinedProfile | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [managedClasses, setManagedClasses] = useState<TrainingClass[]>([]);
  const [enrolledClass, setEnrolledClass] = useState<TrainingClass | null>(null);
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      setProfile(data as UserProfile | CoachProfile | StudentProfile | CombinedProfile);
      ...
      if (hasRole('Coach') && 'managedClasses' in data) {
        ...
        stats.totalStudents = data.managedClasses?.reduce((sum: number, cls: any) => sum + (cls.enrolledStudents?.length || 0), 0);
  ...
  const fetchManagedClasses = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch managed classes');
    }
  }, [token, hasRole, apiCall]);
  ...
  const fetchEnrolledClass = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch enrolled class');
    }
  }, [token, hasRole, apiCall]);
  ...
  const fetchPaymentHistory = useCallback(async () => {
    ...
    try {
      ...
      const upcoming = payments.find(p => p.status === 'Pending' && new Date(p.dueDate) > new Date());
  ...
  // Create payment
  const createPayment = useCallback(async (data: PaymentFormData): Promise<MercadopagoPayment | null> => {
  ...
  const markAttendance = useCallback(async (classId: number, studentId: number, present: boolean, notes?: string): Promise<boolean> => {
  ...
  }, [token, hasRole, fetchProfile, fetchManagedClasses, fetchEnrolledClass, fetchPaymentHistory]);
